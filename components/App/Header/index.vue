...
onMounted(() => {
  setTimeout(() => {
    changeIndicatorActiveTabOnScroll((title) => (animateHeaderIndicator(title)))
  }, 1500)

  window.addEventListener('resize', useDebounceFn(() => animateHeaderIndicator(toValue(activeTab), true), 750))
})
  ...
  <header id='header' class='fixed z-10 bottom-0 left-1/2 -translate-x-1/2  sm:-translate-y-3 max-w-full'>
    <div class="header__container min-h-40 overflow-hidden w-full flex items-end">
      <div class="px-8 max-w-full rounded-t-xl  sm:rounded-full bg-main-foreground">
        <div class="header__wrapper h-24 z-20 py-3 relative w-full flex items-center justify-center">
          <div id="indicator"
          ...
          <template v-for="(link, index) in HEADERLINKS">
            <div class="head-link transition-all w-20 cursor-pointer flex flex-col relative items-center justify-center"
              ...
              <span v-text="link.title" class="text-white select-none text-xs h-0 scale-0 leading-none head-text capitalize"/>
            </div>
          </template>
        </div>
      </div>
